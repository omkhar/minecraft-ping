...
func TestRunWithRuntimeWritesUsageOnInvalidArgv(t *testing.T) {
	...
	for _, argv := range tests {
		...
		if stdout.Len() != 0 {
			t.Fatalf("runWithRuntime(%v) stdout = %q, want empty", argv, stdout.String())
		}
		if !strings.Contains(stderr.String(), "Usage: minecraft-ping [options] destination") {
			t.Fatalf("runWithRuntime(%v) stderr = %q", argv, stderr.String())
		}
	}
}
...
func TestRunWithRuntimeJSONProbe(t *testing.T) {
	...
	if result.Server != "example.com" || result.LatencyMs != 1 {
		t.Fatalf("result = %+v", result)
	}
}

func TestDurationToLatencyMs(t *testing.T) {
	tests := []struct {
		name   string
		input  time.Duration
		wantMS int
	}{
		{name: "sub millisecond", input: 500 * time.Microsecond, wantMS: 1},
		{name: "exact millisecond", input: time.Millisecond, wantMS: 1},
		{name: "multiple milliseconds", input: 2500 * time.Microsecond, wantMS: 2},
	}

	for _, test := range tests {
		if got := durationToLatencyMs(test.input); got != test.wantMS {
			t.Fatalf("%s: durationToLatencyMs(%s) = %d, want %d", test.name, test.input, got, test.wantMS)
		}
	}
}
...
func TestRunTextSessionPreservesInjectedClockWhenSleepIsNil(t *testing.T) {
	var stdout bytes.Buffer
	probe := &stubPreparedProbe{
		bannerText:  "PING example.com (203.0.113.10) port 25565 [java]:",
		summaryText: "example.com",
		samples: []probeSample{{
			...
			remote:  mustAddrPort(t, "203.0.113.10:25565"),
		}},
	}
	...
	want := "[1700000000.123000] pong from 203.0.113.10:25565: seq=1 time=1.000 ms"
...
func TestRunWithRuntimePreservesCustomNowWhenSleepMissing(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	now := time.Unix(123, 456_000_000)
	probe := &stubPreparedProbe{
		bannerText:  "PING example.com (203.0.113.10) port 25565 [java]:",
		summaryText: "example.com",
		samples: []probeSample{
	...
	if !strings.Contains(stdout.String(), "[123.456000] pong from 203.0.113.10:25565: seq=1 time=12.000 ms") {
