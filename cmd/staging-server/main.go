...
func main() {
	...
	flag.StringVar(&cfg.BedrockListenIPv4, "bedrock-listen4", "", "Bedrock IPv4 listen address, or empty to disable")
	flag.StringVar(&cfg.BedrockListenIPv6, "bedrock-listen6", "", "Bedrock IPv6 listen address, or empty to disable")
	flag.StringVar(&cfg.StatusJSON, "status-json", stagingserver.DefaultStatusJSON(), "Status response JSON")
	flag.StringVar(&cfg.BedrockStatus, "bedrock-status", "", "Bedrock status response (defaults to a generated MCPE response when Bedrock listeners are enabled)")
