...
func (rt sessionRuntime) withDefaults() sessionRuntime {
	defaults := defaultSessionRuntime()
	if rt.now == nil {
		rt.now = defaults.now
	}
	if rt.sleep == nil {
		rt.sleep = defaults.sleep
	}
	return rt
}
...
func runTextSession(ctx context.Context, stdout io.Writer, cfg cliConfig, probe preparedProbe, rt sessionRuntime) int {
	...
	for {
		...
		sleepFor := max(cfg.Interval-rt.now().Sub(probeStart), 0)
